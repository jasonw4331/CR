...
class Main extends PluginBase {
	...
	public static function getRomanNumber(int $integer) : string {
		$characters = ['M' => 1000, 'CM' => 900, 'D' => 500, 'CD' => 400, 'C' => 100, 'XC' => 90, 'L' => 50, 'XL' => 40, 'X' => 10, 'IX' => 9, 'V' => 5, 'IV' => 4, 'I' => 1];
		$romanString = "";
		while($integer > 0) {
	...
	public function onEnable() : void {
		...
		foreach($this->getConfig()->getNested("Power-Areas.Areas", []) as $areaKey => $areaData) {
			...
			if($level === null) {
				continue;
			} // skip if level isn't loaded
			$this->areas[$areaKey] = new Area($areaData["x1"], $areaData["z1"], $areaData["x2"], $areaData["z2"], $level, $areaData["claimed"]);
		}
		$this->getServer()->getScheduler()->scheduleRepeatingTask(new ClaimProgressDisplayTask($this), 35 // 1.75 seconds
		);
		$this->getServer()->getScheduler()->scheduleRepeatingTask(new PowerGiveTask($this), (20 * (int) $this->getConfig()->getNested("Power-Areas.Time-Per-Power", 120)) // 2 minutes
		);
		$this->getServer()->getScheduler()->scheduleRepeatingTask(new AreaCheckTask($this), (20 * (int) $this->getConfig()->getNested("Power-Areas.Area-Check-Time", 15)) // 15 seconds
		);
	...
	public function onDisable() : void {
		...
		foreach($this->areas as $areaKey => $area) {
			$this->getConfig()->setNested("Power-Areas.Areas.".$areaKey, ["x1" => (int) $area->minX, "x2" => (int) $area->maxX, "z1" => (int) $area->minZ, "z2" => (int) $area->maxZ, "level" => $area->getLevel()->getFolderName(), "claimed" => $area->getClaimer()]);
		}
		$this->getConfig()->save();
	}
